"""
Simplified Multi-Agent Blog System (Groq version)
Faster (no evaluation, no readability scoring)
"""
...
def simple_search(topic: str, max_results: int = 5) -> List[Article]:
    ...
    try:
        ...
        return [
            ...
            for r in organic[:max_results] if r.get("title") and r.get("link")
        ]
    except Exception:
        return []
...
def research_agent(state: BlogState) -> BlogState:
    ...
    if not topic:
        return state
    print(f"🔍 Researching (Groq): {topic}")
...
def writer_agent(state: BlogState) -> BlogState:
    ...
    print(f"✍️ Writing blog (Groq): {topic}")
    prompt = ChatPromptTemplate.from_messages([
...
def image_agent(state: BlogState) -> List[ImageData]:
    if not PEXELS_API_KEY:
        return []
    topic = state.get("topic", "")
    print(f"🖼️ Searching images (Groq) for: {topic}")
    try:
        res = requests.get(
