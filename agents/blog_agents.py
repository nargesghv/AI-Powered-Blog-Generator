...
def summarizer_agent(state: BlogState) -> BlogState:
    ...
    prompt = f"""you are a specialist in context undestanding and summerizing.Summarize the following research results into key points and themes for a blog on: {state['topic']}\n\n{content}"""
...
def writer_agent(state: BlogState) -> BlogState:
    ...
    prompt = f"""
You are a professional blog writer. Write a high-quality, 1000-word blog post in **Markdown format** on the topic: "{state['topic']}".
Use the following research summary as your guide:
{summary}
Include headings, bullet points, conclusion and links where relevant.
Follow these guidelines:
- Use the research summary below as the factual basis and core structure.
- Write in a professional, engaging tone appropriate for a tech-savvy audience.
- Structure the post with clear **headings** and **subheadings**.
- Use **bullet points**, **bold text**, and **code snippets** where relevant.
- Add a strong **introduction**, well-organized **sections**, and a **conclusion**.
- Where appropriate, include **external links** to credible sources.
"""
